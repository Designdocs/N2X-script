...
ExecStartPre=/bin/sh -c 'test -n "$N2X_API_HOST" || { echo "N2X_API_HOST is empty; set it in /etc/N2X/.env"; exit 1; }'
ExecStartPre=/bin/sh -c 'test -n "$N2X_API_KEY" || { echo "N2X_API_KEY is empty; set it in /etc/N2X/.env"; exit 1; }'
